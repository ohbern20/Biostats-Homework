...
Now I will get a quick visual of the variables (since we already examined the summary() of the data above)
I will create a histogram of worms first
```{r}
...
Now I will plot both variables together
```{r}
ggplot(worms, aes(x = Forest, y = Worms, color = Forest)) +
  geom_boxplot() +
  theme_bw() 
```
From looking at this graph I am guessing that there will be a significant difference between the amount of worms found in Kip vs. Donnerville. It apears there are more worms in kip. 

I will now fit the data to a matrix
```{r}
worms.mat <- xtabs(Worms ~ Forest,
                  data = worms)
```

And I will check the assumption of the model. I am not sure how to check the assumptions for this type of test, autoplot() does not seem to be worknig and I can't find an alternative method in GSWR. For now I will just move forward with the test.
...
I reject the null hypothesis and accept that there is a significant difference between the abundance of worms in the Kip Tract compared to Donnerville.
...
From observing this graph, it looks like there is a difference between the amount of Mg found in these forests, however there is also overlap in the amounts so it may not be significant. 
...
Again, I'm not sure how to check the assumptions for this type of test. I will plan to ask about this in class and then revise. 

I will now run the test
```{r}
t.test(Mg_ppm ~ Forest, data = mg) #large p-value! There level of Mg in the soil is not significantly different between the two locations. 
```

I accept the null hypothesis that there is no difference in Mg between the two forest locations. 
...
ggplot(soil, aes(x = Zn_ppm)) +
  geom_histogram() #there is a wide range in the data but nothing that makes me uncomfortable given that there are a few values consistantly higher than the mean
```
...
Prediction is that there is a significant difference in the levels of Zn between the forests. Seems like the prediction is supported in this figure
